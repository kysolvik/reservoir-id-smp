...
data_path=gs://res-id/ee_exports/landsat5_30m_v1/1985
region_shp_path='gs://res-id/cnn/aux_data/Brazil_10kmbuffer_noremoteislands_noholes.*'

pred_zip_gs_path=gs://res-id/cnn/predict/landsat_v1/ls5_1985_preds.zip
...
# Get and unzip previous preds
sudo apt install -y zip

gsutil cp $pred_zip_gs_path ./
unzip *.zip
tsp bash quick_swap_num_order.sh
...
# Run
tsp python3 -u predict_smp_landsat.py ls_10m.vrt model.ckpt mean_std.npy bands_minmax.npy out2/ --quantized --region_shp shp/Brazil_10kmbuffer_noremoteislands_noholes.shp
