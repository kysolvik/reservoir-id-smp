...
# Input dir with individual pred tiles
IN_DIR = sys.argv[1] # Example: './out/out_allbrazil_v12_30m/sentinel/2021'
# Output dir to store processed, saved tifs, each PROCESS_BOX_SIZE x PROCESS_BOX_SIZE
OUT_DIR = sys.argv[2] # Example: './out/merged'

# Side-length for processing tiles (10k to 20k seems pretty good)
PROCESS_BOX_SIZE = 20000
# If true, will create 'full_overlapped.tif' in OUT_DIR that is full mosaiced output
CREATE_FULL_TIF = True
...
def filter_bounds(tile_bounds, rows, cols):
    ...
    Args:
        ...
        rows (list): List of integer row starts
        cols (list): List of integer col starts
    """
...
def main():
    ...
    vrt_info = template_vrt_info(file_list, make_vrt=True)
    ...
    if CREATE_FULL_TIF:
        ...
        sp.call(
            ['gdalbuildvrt', os.path.join(OUT_DIR, 'full_overlapped.vrt')]
            + glob.glob(os.path.join(OUT_DIR, 'overlapped_*.tif'))
            )
        sp.call([
            'gdal_translate',
            '-co', 'COMPRESS=LZW',
